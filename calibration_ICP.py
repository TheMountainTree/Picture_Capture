...
def average_transforms_robust(transforms, infos):
    ...
    print(f"    保留帧的索引: {valid_indices.tolist()}")
    print(f"    保留帧的误差: {valid_errors}")
...
def main():
    ...
    angle_deg = np.linalg.norm(r.as_rotvec()) * 180 / np.pi
    euler = r.as_euler('xyz', degrees=True)
    
    print("\n" + "="*60)
    print("最终结果 (Orbbec -> Kinect)")
    print("="*60)
    print(T_final)
    print(f"\n旋转: {angle_deg:.2f}°")
    print(f"  欧拉角: [{euler[0]:.1f}°, {euler[1]:.1f}°, {euler[2]:.1f}°]")
    print(f"\n平移: {np.linalg.norm(t):.3f} m ({np.linalg.norm(t)*100:.1f} cm)")
    print(f"  向量: [{t[0]:.3f}, {t[1]:.3f}, {t[2]:.3f}]")
    ...
    # 验证
    print("\n生成验证点云...")
    files_pcd = sorted(glob.glob(os.path.join(DIR_ORBBEC, "orbbec_cloud_*.ply")))
    if len(files_pcd) > 0:
        pcd_o = o3d.io.read_point_cloud(files_pcd[0])
        
        # Transform Orbbec cloud to Color frame first
        pts_depth = np.asarray(pcd_o.points)
        pts_h = np.hstack([
            pts_depth,
            np.ones((pts_depth.shape[0], 1))
        ])
        pts_color = (T_depth2color @ pts_h.T).T[:, :3]
        pcd_o.points = o3d.utility.Vector3dVector(pts_color)
        
        ts = get_timestamp(files_pcd[0], "orbbec")
        pcd_k_path = os.path.join(DIR_KINECT, f"kinect2_cloud_{ts}.ply")
        
        if os.path.exists(pcd_k_path):
            pcd_k = o3d.io.read_point_cloud(pcd_k_path)
            
            pcd_o.transform(T_final)
            pcd_o.paint_uniform_color([1, 0, 0])
            pcd_k.paint_uniform_color([0, 1, 0])
            
            merged = pcd_o + pcd_k
            out_path = os.path.join(BASE_DIR, "verify_symmetric.ply")
